# Deployment Options - Choose one(with repo cloning - ci/cd)
## Deploy with Railway (with VPS - better performance)
[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/template/UWddwE?referralCode=Uc3pOA)

## Deploy with Vercel (serverless - Cold Start - not optimized for performance - fast deployment)

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/barkarian/payload-website-template&env=PAYLOAD_SECRET%2CNEXT_PUBLIC_SERVER_URL%2CPAYLOAD_PUBLIC_SERVER_URL&build-command=yarn%20run%20build&stores=%5B%7B%22type%22%3A%22postgres%22%7D%2C%7B%22type%22%3A%22blob%22%7D%5D)

1) Create a project on Vercel by following this instructions (keep NEXT_PUBLIC_SERVER_URL ="WHATEVER" initially)
2) After setup,
  1) Get the WEBSITE_URL from the project settings (e.g. https://my-domain.vercel.app)
  set NEXT_PUBLIC_SERVER_URL,PAYLOAD_PUBLIC_SERVER_URL env variables to your domain (WEBSITE_URL)
  2) on Settings>BUILD command add :
  ```
  yarn build
  ```
  3) on Settings>INSTALL command add :
  ```
  yarn install
  ```
## Quick Start
1) To spin up this example locally, follow these steps clone this repo:
```
git clone https://github.com/barkarian/payload-start.git example-project-name
 ...
 ** (RECOMMENDED WAY) **
 ** With the ./start-database - uses docker under the hood: **
  ```
  ./start-database.sh
  ```
  ** (ALTERNATIVE WAY) **
  ** Just setup a database on your own. **
